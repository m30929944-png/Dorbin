...
app.use(cors());
app.use(bodyParser.json({ limit: '15mb' })); // برای عکس پروفایل base64
app.use(express.static(__dirname));

const db = new DatabaseManager();

// ============================================
// ثبت‌نام / ورود ساده (بدون رمز عبور - شناسه در مرورگر ذخیره می‌شود)
// ============================================
...
// دریافت اطلاعات پایه‌ی کاربر (برای هدر)
...
// آپلود / تغییر عکس پروفایل (به‌صورت base64 ذخیره می‌شود)
...
// ============================================
// پروفایل عمومی یک کاربر (برای اکسپلور و بازدید از پروفایل دیگران)
// ============================================
...
// ============================================
// ساخت پست جدید در کانال (منتشر فوری)
// ============================================
...
// چت عمومی با دستیارِ یک کاربر دیگر — فقط پاسخ می‌دهد، هیچ داده‌ی آموزشی خام نشان نمی‌دهد
...
// اکسپلور — رتبه‌بندی بر اساس فعالیت واقعی (boost_level / activity_score)، نه فالوور تقلبی
...
io.on('connection', (socket) => {
    ...
    socket.on('private_message', async (data) => {
        ...
        // اگر گیرنده آنلاین نبود هم پیام ذخیره شود
        try {
            const id = crypto.randomUUID();
            await db.query(from, `INSERT INTO messages (id, from_user, to_user, message) VALUES ($1, $2, $3, $4)`, [id, from, to, message]);
