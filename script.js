...
async function loadChannelPosts() {
    try {
        ...
        document.getElementById('channelPosts').innerHTML = posts.length ? 
    ...
    if (box.classList.contains('open') && !box.dataset.loaded) {
        ...
        `).join('') || '') + `
            <div class="comment-form">
                <input type="text" id="commentInput-${postId}" placeholder="کامنت بنویس..." onkeydown="if(event.key==='Enter') submitComment('${postId}')">
...
// زمان‌بندی پست‌ها با پشتیبانی از ویدیو و عکس
let scheduledMediaFiles = [];
...
async function schedulePosts() {
    ...
    baseDate.setHours(parseInt(time.split(':')[0]), parseInt(time.split(':')[1]), 0, 0);
...
async function sendMessage() {
    ...
    // رمزنگاری در سمت کلاینت برای امنیت بیشتر
...
async function adminAction(type, id, action) {
    ...
    try {
        ...
        if (data.success) {
            ...
            loadAdminData(document.querySelector('.admin-tab.active').dataset.tab);
        }
